...
const wheels=[],front=[];for(const x of [-1,1])for(const z of [-1.3,1.3]){const pivot=new THREE.Group();pivot.position.set(x,.48,z);car.add(pivot);const wheel=new THREE.Mesh(new THREE.CylinderGeometry(.45,.45,.3,20),black);wheel.rotation.z=Math.PI/2;wheel.castShadow=true;pivot.add(wheel);const hub=new THREE.Mesh(new THREE.CylinderGeometry(.23,.23,.315,12),mat('#b6c6bd',.4));hub.rotation.z=Math.PI/2;pivot.add(hub);
// Spokes and an asymmetric marker make rolling speed and direction visible.
for(const side of [-1,1]){
  for(let i=0;i<5;i++){const a=i*Math.PI*2/5;const spoke=new THREE.Mesh(new THREE.BoxGeometry(.2,.016,.045),black);spoke.position.set(Math.cos(a)*.11,side*.166,Math.sin(a)*.11);spoke.rotation.y=-a;hub.add(spoke);}
  const marker=new THREE.Mesh(new THREE.BoxGeometry(.1,.02,.06),head);marker.position.set(.19,side*.18,0);hub.add(marker);
  const tireMark=new THREE.Mesh(new THREE.BoxGeometry(.065,.012,.09),white);tireMark.position.set(.37,side*.157,0);wheel.add(tireMark);
}
wheels.push(wheel,hub);if(z>0)front.push(pivot);}
...
window.driveState=()=>({speed,heading,x:car.position.x,z:car.position.z,paused,lap,lapTime,camMode,nextCheckpoint,steer,wheelAngle:front[0].rotation.y,wheelRoll:wheels[0].rotation.x});
